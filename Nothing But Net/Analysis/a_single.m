...
for itest = ITEST
  ...
  lbl = sprintf('%s test-%d AVGBAT=%.2f ',dataName, itest, vbat(1));
   ...
   subplot(2,3,2);box on; grid on; hold on; xlabel('ANG'),ylabel('VCMD'); title(lbl);  xlim([-160 270])
   plot(ang,vcmd,ls{itest});
       % xlim([0 250]);
       xlim([-150 200]);


   subplot(2,3,1);box on; grid on; hold on; xlabel('TIME'),ylabel('VCMD'); xlim(xrange); title(lbl);
  ...
  subplot(2,3,5);box on; grid on; hold on; xlabel('ANG'),ylabel('RPM');
  % xlim([0 250]); 
       xlim([-150 200]);
