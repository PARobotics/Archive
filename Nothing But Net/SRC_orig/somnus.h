...
int BAT_MODE      = SINGLE;     // SINGLE/CONTINUOUS
int LIFT_MODE     = OFF;        // ON/OFF
int SHIFT_ON      = 0;          // == 1 (push & release 7L) is the shift key
...
int AUTO_ADJUST = OFF;          // allow auto adjustment in continuous mode
...
// int     Intaker_rpm       = 0;    // RPM value in 10xRPM
// int     Intaker_t_last    = 0;    // cache of the time at last monitor frame (used to calculate rpm)
// int     Intaker_t0        = 0;    //
// int     Intaker_val_last  = 0;     // cache of val1 of the last frame (used to calculate rpm)
...
#define MOVE_X_KV  2
// #define MOVE_X_BRAKE_V1   180 // speed deceeleration curve
int MOVE_X_BRAKE_V0 = 180; // speed deceeleration curve
...
void move_x(int vol)   ;
void move_hold(int option);
void rotate(int  vol)  ;
void bat(int vol)      ;
void intaker(int vol)  ;
void feeder(int vol)   ;
void move_x_by_slow(int d, int tlimit);
void move_x_by(int d, int tlimit);
void rotate_by(int angle, int tlimit);
void rotate_by_slow(int angle, int tlimit);
