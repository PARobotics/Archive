...
void lcd_mode_selection()
{
  ...
  // now select auton/program skills mode -- update display accordingly
    ...
    if (MODE<0) {MODE=6;}    // LEFT and RIGHT arrow circles 0->5
    if (MODE>6) {MODE=0;}
...
void lcd_message()
{
  ...
  if (DIST==SHORT) sprintf(lcdStr1, "%1d%1d%1d %2d %2d %2d ", LIFT_MODE,BAT_MODE-100,DIST-100,volt1()/100,volt2()/100, BAT_SHORT_HIT_V);
  if (DIST==MEDIUM) sprintf(lcdStr1, "%1d%1d%1d %2d %2d %2d ", LIFT_MODE,BAT_MODE-100,DIST-100,volt1()/100,volt2()/100, BAT_MEDIUM_HIT_V);
  if (DIST==LONG) sprintf(lcdStr1, "%1d%1d%1d %2d %2d %2d ", LIFT_MODE,BAT_MODE-100,DIST-100,volt1()/100,volt2()/100, BAT_LONG_PULL_ANG_MAX);
