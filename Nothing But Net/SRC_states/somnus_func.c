...
// -- apply a small voltage to hold in one of the 4 positions
//    FWD/BKWD/LEFT/RIGHT
void move_hold(int option) {
  if (option==FWD)   move_x(  MOVE_HOLD_V );
  if (option==BKWD)  move_x( -MOVE_HOLD_V );
  if (option==LEFT)  rotate( -MOVE_HOLD_V );
  if (option==RIGHT) rotate(  MOVE_HOLD_V );
}
