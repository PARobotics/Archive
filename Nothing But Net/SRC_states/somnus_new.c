...
task usercontrol()
{
    ...
    while(true){
        ...
        if(MODE == AUTO_A || MODE == AUTO_B || MODE == AUTO_C || MODE == RBT_SKILL || MODE == TEST_B ){
            ...
            if(vexRT[Btn8RXmtr2] == 1 && LIFT_MODE == OFF){
                if (BAT_MODE==SINGLE && DIST==SHORT) BAT_CMD = SHORT;
                if (BAT_MODE==SINGLE && DIST==SKILLS) BAT_CMD = SKILLS;
                if (BAT_MODE==SINGLE && DIST==MEDIUM) BAT_CMD = SKILLS;
