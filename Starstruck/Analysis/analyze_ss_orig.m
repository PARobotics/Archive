function analyze_autoa(rawFile)
%
...
% dos(['grep  -iaE "^W " ' rawFile 'fa| gawk ''{if (NF==6) print $2,$3,$4,$5,$6} '' > ' datFile1]);
...
% remove the bad data points and initial time to zero
if (length(pos)>0)
  pos = clean_data(pos,100*ones(size(dmp,2),1));    %clean the data 
...
if length(dmp) >0; 
  ...
  % dmp = dmp(:,3:4);            % in deg and rpm
 

  figure(2)
  box on; grid on; hold on; 
  plot(t_dmp,dmp(:,1),'b-+');
  plot(t_dmp,dmp(:,2)*3.125,'r-+');
  plot(t_dmp,dmp(:,3),'g-+');
  title(lbl, 'interpreter','none','fontweight','b');
